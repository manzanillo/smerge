...
const CommitList: React.FC<Props> = ({
  data = [{ author: "", date: "", msg: "", hash: "" }],
  branch,
  isLoading,
  perPage = 10,
}) => {
  const [renderList, setRenderList] = useState<Commit[]>([
    { author: "", date: "", msg: "", hash: "" },
  ]);
  ...
  return (
    <Stack spacing={3} alignContent={"center"} alignItems={"center"}>
      ...
      >
        {renderList.length > 0 && skel && !isLoading
          ? renderList.map((commit, index) =>
              customListItem(
                currentPage.current + "-" + index.toString(),
                commit,
                branch,
          ...
          : range(0, perPage, 1).map((index) => (
              <div key={"skel-" + index} style={{ width: "800px" }}>
                <ListItem
                  sx={{ pt: "10px", pr: "10px", pb: "7px", pl: "15px" }}
                >
...
const customListItem = (
  ...
  return (
    <div className="commitListDiv" key={key} style={{ minWidth: "800px" }}>
          ...
          <Grid container justifyContent={"center"}>
            <ClickAwayListener onClickAway={() => handleExpand()}>
              <Button
                ...
                variant="outlined"
              >
                Switch dev server to this commit
              </Button>
            </ClickAwayListener>
      ...
      {!isLast ? (
        <Divider variant="inset" component="li" sx={{ mr: "15px" }}></Divider>
      ) : (
        <></>
      )}
    </div>
