...
const Login = ({ onAuthSuccess }: loginProps) => {
  ...
  const validatePassword = (passwordVal: string, passwordAgainVal: string) => {
    setPasswordAgainError(
      passwordVal != passwordAgainVal && passwordAgainVal != ""
    );
    setPasswordAgainErrorText(
      passwordVal != passwordAgainVal
        ? passwordAgainVal == ""
          ? ""
  ...
  const styles = {
    paperContainer: {
      backgroundImage: `radial-gradient(ellipse at center,rgba(0, 0, 0, 0) 80%,rgba(0, 0, 0, 0.8) 100%),url(https://images.pexels.com/photos/2080963/pexels-photo-2080963.jpeg?auto=compress&cs=tinysrgb&h=${Math.max(
        window.innerHeight,
        1080
      )})`,
            ...
            <Stack direction={"column"} spacing={4}>
              ...
              <form onSubmit={handleSubmit}>
                <FormControl>
                  ...
                  {!isRegister ? (
                    <FormControlLabel
                      control={
                        <Checkbox onChange={(_, val) => setRemember(val)} />
                      }
                      label="Remember Me"
                    />
                  ) : (
              ...
              {isRegister ? (
                <>
                  <small>
                    You have a account?{" "}
                    <Link onClick={clearErrors} to="/access/login">
                      Login here
                    </Link>
                  </small>
                </>
