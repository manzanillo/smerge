...
def generateSyncBlockXML():
    with open("snapmerge/static/snap/simple_sync_block.js", "r") as f:
        content = f.read()
    saveContent = make_xml_safe(content)
    with open("snapmerge/static/snap/simple_sync_block_blank.xml", "r") as f:
        blankFile = f.read()
    with open("snapmerge/static/snap/simple_sync_block.xml", "w") as f:
        f.write(blankFile.replace("{{js_code}}", saveContent))
